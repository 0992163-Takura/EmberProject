...
void CActor::ProcessEvent(SEntityEvent& event)
{
    switch (event.event)
        ...
        {
            PrefetchLine(m_boneTrans, 0);
            PrefetchLine(m_boneTrans, 128);
            PrefetchLine(m_boneTrans, 256);
            PrefetchLine(m_boneTrans, 384);
...
bool CActor::IsRemote() const
{
    ...
    return gEnv->bMultiplayer && !IsClient() && !(gEnv->bServer && (myClass == sDummyPlayerClass));
}
            ...
            for (uint32 boneId = 0; boneId < BONE_ID_NUM; ++boneId)
                {
                    ...
                    if (!sName.empty())
                        {
                            m_boneIDs[boneId] = pSkelPose->GetJointIDByName(sName.c_str());
    ...
    if(m_pAnimatedCharacter)
        {
            ...
            switch(arms)
                {
                case 1:
                    animCharArms = eACA_RightArm;
                    break;
                case 2:
                    animCharArms = eACA_LeftArm;
                    break;
                default:
                    animCharArms = eACA_BothArms;
                }
    ...
    if (pCharacter)
        {
            ...
            newLimb.SetLimb(limbInfo.characterSlot,
                            ...
                            pCharacter->GetISkeletonPose()->GetJointIDByName(limbInfo.sRootBone.c_str()),
                            pCharacter->GetISkeletonPose()->GetJointIDByName(limbInfo.sMidBone.c_str()),
                            pCharacter->GetISkeletonPose()->GetJointIDByName(limbInfo.sEndBone.c_str()),
                            limbInfo.flags);
                    ...
                    if (pCharPhys->GetParams(&ppart))
                        {
                            ...
                            pab.bGrounded=1;
                            pab.pHost=pPhysicalEntity;
                            m_pLegsFrame->SetParams(&pab);
                            pe_articgeomparams agp;
                            agp.flags=agp.flagsCollider = 0;
                            agp.idbody=0;
                            ...
                            for(int i=0; i<2; i++,boneName[6]='R')
                                if ((ppart.partid=pChar->GetISkeletonPose()->GetJointIDByName(boneName))>0 && (ppart.ipart=-1,pCharPhys->GetParams(&ppart)))
                                    {
                                        QuatT qbone = pChar->GetISkeletonPose()->GetAbsJointByID(m_iboneLeg[i]=ppart.partid);
                                        pp.pos = pos + q*qbone.t;
                                        pp.q = q*qbone.q;
                                        ...
                                        aac.pt[0].zero();
                                        aac.qframe[0].SetIdentity();
                                        aac.pt[1] = q*qbone.t;
                                        aac.qframe[1] = q*qbone.q;
                                        aac.maxPullForce = aac.maxBendTorque = 5000.0f;
                                        aac.pBuddy = m_pLegsFrame;
                                        aac.id = 100;
            ...
            else for(int i=0; i<2; i++)	if (m_pLegsCollider[i])
                        {
                            ...
                            if (bActive!=m_bLegActive[i])
                                {
                                    pe_params_flags pf;
                                    pf.flagsOR=pef_traceable&-bActive;
                                    pf.flagsAND=~(pef_traceable&~-bActive);
                                    m_pLegsCollider[i]->SetParams(&pf);
                                    pf.flagsOR=pef_disabled&~-bActive;
                                    pf.flagsAND=~(pef_disabled&-bActive);
                            ...
                            if (bActive)
                                {
                                    pe_action_update_constraint auc;
                                    auc.idConstraint = 100;
                                    auc.flags = local_frames;
                                    auc.pt[1] = q*qbone.t;
