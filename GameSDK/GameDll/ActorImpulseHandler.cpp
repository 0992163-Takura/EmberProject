    ...
    if (pSkelPose)
        {
            for (int i=0; i<NUM_JOINTS; i++)
                {
                    m_jointIdx[i] = pSkelPose->GetJointIDByName(::JOINT_NAMES[i]);
                }
        }
            ...
            if (strcmp(pChild->GetName(), "ImpulseSet") == 0)
                {
                                    ...
                                    if (bHasDirection && bHasStrenght)
                                        {
                                            ...
                                            if (szPartID && szPartID[0])
                                                {
                                                    const int iPartId = pSkelPose->GetJointIDByName(szPartID);
                                                    if (iPartId != -1)
                                                        {
                    ...
                    // partIds
                    {
                        ...
                        while (!sPartID.empty())
                            {
                                const int iPartID = pSkelPose->GetJointIDByName(sPartID.c_str());
                                if (iPartID != -1)
                                    {
