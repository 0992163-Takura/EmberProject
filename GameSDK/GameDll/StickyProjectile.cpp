...
struct SStuckSer
{
    ...
    bool IsStuck() const
    {
        return pThis->IsStuck();
    }
    void SetStuck(bool stuck)
    {
        return pThis->NetSetStuck(pProjectile, stuck);
    }
            ...
            if (!pActor || (stickParams.m_bStickToFriendlies || !pActor->IsFriendlyEntity(stickParams.m_ownerId)) && (gEnv->bMultiplayer || !pActor->IsDead()))
                {
                    ...
                    const char* boneName = pSkeleton->GetJointNameByID(m_stuckJoint);
            ...
            if(pTargetEntity)
                {
                    if(ICharacterInstance* pTargetCharacter = pTargetEntity->GetCharacter(0))
                        {
                            const char* boneName = pTargetCharacter->GetISkeletonPose()->GetJointNameByID(m_stuckJoint);
