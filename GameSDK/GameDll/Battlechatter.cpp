...
struct SBattlehchatterAutoComplete : public IConsoleArgumentAutoComplete
{
    ...
    {
        return s_battlechatterName[nIndex];
    };
};
...
void CBattlechatter::Init()
{
    ...
    bool found = g_pGame->GetIGameFramework()->GetNetworkSafeClassId(m_gaussWeaponClassId, "Gauss");
    CRY_ASSERT(found);
    found = g_pGame->GetIGameFramework()->GetNetworkSafeClassId(m_dsg1WeaponClassId, "DSG1");
    CRY_ASSERT(found);
    found = g_pGame->GetIGameFramework()->GetNetworkSafeClassId(m_jawWeaponClassId, "JAW");
    CRY_ASSERT(found);
    found = g_pGame->GetIGameFramework()->GetNetworkSafeClassId(m_HMGClassId, "HMG");
    CRY_ASSERT(found);
}
    ...
    if(m_clientPlayer && hitInfo.shooterId && hitInfo.targetId)
        {
            ...
            if(actorId)
                {
                    if(hitInfo.weaponClassId == m_gaussWeaponClassId || hitInfo.weaponClassId == m_dsg1WeaponClassId)
                        {
                            Event(BC_Sniper, actorId);
                        }
                    else if(hitInfo.weaponClassId == m_jawWeaponClassId)
                        {
                            Event(BC_MissileCountermeasures, actorId);	//Now also for vanilla jaw usage
                        }
                    else if (hitInfo.weaponClassId == m_HMGClassId)
                        {
                            Event(BC_Enemy_HMG, actorId);
                        }
                    else
                        {
