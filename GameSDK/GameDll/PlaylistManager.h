...
struct SPlaylist
{
    ...
    {
        return rotExtInfo.localName.c_str();
    }
    ILINE const char* GetDescription() const
    {
        return rotExtInfo.localDescription.c_str();
    }
    ILINE const char* GetUniqueName() const
    {
        return rotExtInfo.uniqueName.c_str();
    }
    ILINE bool  IsEnabled() const
    {
        return rotExtInfo.m_enabled;
    }
    ILINE bool  IsHidden() const
    {
        return rotExtInfo.m_hidden;
    }
    ILINE void  SetEnabled(bool e)
    {
        rotExtInfo.m_enabled = e;
    }
    ILINE bool	RequiresUnlocking() const
    {
        return rotExtInfo.m_requiresUnlocking;
    }
    ILINE bool  IsSelectable() const
    {
        return (!bSynchedFromServer);
    }
...
public:
    ...
    bool IsGameModeSpecific()
    {
        return m_gameModeSpecific;
    }
...
public:
    ...
    ILINE bool  HavePlaylistSet() const
    {
        return (m_currentPlaylistId != 0);
    }
    ...
    ILINE bool IsUsingCustomVariant()
    {
        return GetActiveVariantIndex() == m_customVariantIndex;
    }
    ...
    const SPlaylist* GetPlaylistByUniqueName(const char *pInUniqueName)
    {
        return FindPlaylistByUniqueName(pInUniqueName);
    }
    const char*			 GetPlaylistNameById(ILevelRotation::TExtInfoId findId)
    {
        return FindPlaylistById(findId)->GetName();
    }
    ...
    const TVariantsVec &GetVariants() const
    {
        return m_variants;
    }
    void GetVariantsForGameMode(const char *pGameMode, TVariantsPtrVec &result);

    ILINE int GetDefaultVariant() const
    {
        return m_defaultVariantIndex;
    }
    ILINE int GetCustomVariant() const
    {
        return m_customVariantIndex;
    }
    ...
    void IgnoreCVarChanges(bool enabled)
    {
        m_bIsSettingOptions = enabled;
    }
