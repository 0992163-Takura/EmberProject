...
public:
    ...
    ILINE const SCollisionTestParams& GetCollisionTestParams() const
    {
        return m_testParams;
    }
    ILINE bool IsBlocked() const
    {
        return (m_queuedRayID != 0);
    }

private:

    ILINE void OnSuccesfulHit(const ray_hit& hitResult)
    {
        CRY_ASSERT(m_pUser);
        m_pUser->OnSuccesfulHit(hitResult);
    }
    ILINE void OnFailedHit()
    {
        CRY_ASSERT(m_pUser);
        m_pUser->OnFailedHit();
    };
