...
struct DLCContents
{
    DLCContents(): bonusID( 0 ), uniqueID( 0 )
    {
        name [0 ] = '\0';
    }
...
public:
    ...
    bool		IsDLCLoaded(int index) const
    {
        return ( (m_loadedDLCs & m_allowedDLCs) & BIT(index)) != 0;
    }
    uint32	GetLoadedDLCs() const
    {
        return m_loadedDLCs & m_allowedDLCs;
    }
    ...
    static bool MeetsDLCRequirements(uint32 requiredDLCs, uint32 loadedDLCs)
    {
        return (requiredDLCs & loadedDLCs) == requiredDLCs;
    }
    ...
    void SetAppliedEntitlements( uint32 entitlements )
    {
        m_appliedDLCStat = entitlements;
    }
    ...
    uint32 GetAppliedDLCBitfield() const
    {
        return m_appliedDLCStat;
    }
    uint32 GetDLCXPToAward() const
    {
        return m_DLCXPToAward;
    }
...
private:
    ...
    bool IsDLCReallyLoaded(int index)
    {
        return ( m_loadedDLCs & BIT(index)) != 0;
    }
