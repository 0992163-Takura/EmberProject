...
public:
    ...
    {
        return m_circularBuffer.capacity()>0;
    }

    void																*Alloc(int inSize, uint8 inType);
    static void													Free(void *inPtr);

    bool																IsDataTruncated()
    {
        return m_numDiscards>0;
    }
    int																	GetTotalSessionMemoryRequests()
    {
        return m_totalBytesRequested;
    }
    int																	GetBufferCapacity()
    {
        return m_circularBuffer.capacity();
    }
...
public:
    ...
    void																SetTime(const CTimeValue &inTime)
    {
        m_time=inTime;
    }
    ...
    void																*operator new(size_t inSize) throw()
    {
        return CCircularBufferStatsStorage::GetDefaultStorage()->Alloc(inSize,eRBPT_TimelineEntry);
    }
    void																*operator new(size_t inSize, CCircularBufferStatsStorage *inStorage) throw()
    {
        return inStorage->Alloc(inSize,eRBPT_TimelineEntry);
    }
    void																operator delete(void *inPtr)
    {
        CCircularBufferStatsStorage::Free(inPtr);
    }
};
...
struct SCircularStatAnyValue
{
    ...
    void									*operator new(size_t inSize, CCircularBufferStatsStorage *inStorage) throw()
    {
        return inStorage->Alloc(inSize,eRBPT_StatAnyValue);
    }
    void									operator delete(void *inPtr, CCircularBufferStatsStorage *inStorage)
    {
        CCircularBufferStatsStorage::Free(inPtr);
    }
    void									operator delete(void *inPtr)
