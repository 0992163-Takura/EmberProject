...
public:
    ...
    {
        return m_firing || m_throwing;
    };

    // If we need more of this secondary functions, we might consider create one common in IFireMode interface
    virtual void Prime();
    virtual bool IsReadyToThrow() const
    {
        return (m_firing && !m_throwing);
    }
    void FumbleGrenade();

    ILINE const Vec3* GetTrajectory() const
    {
        return m_trajectory;
    }
    ILINE const uint32 GetTrajectoryLength() const
    {
        return m_trajectoryLength;
    }

    ILINE const float	GetExplodeTime() const
    {
        return m_explodeTime;
    }
    ILINE const float GetProjectileLifeTime() const
    {
