...
bool CPlayer::Init(IGameObject * pGameObject)
{
	...
	rAction = new rappleAction(pEntity);

	CryLogAlways("Loaded Past rACtion");
...
void CPlayer::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	rAction->attemptSwing(frameTime);
...
void CPlayer::SetIK( const SActorFrameMovementParams& frameMovementParams )
{
	...
	if (IsThirdPerson())
         ...
         float WeaponMaxDist = g_pGameCVars->cl_tpvMaxWeapDist/3; //This variable in the Cvars dictates weapon and raycast max range, weapon will shoot further but without a raycast hit it will be inacurate
         Vec3 pos = curMovementState.weaponPosition;
         Matrix34 ViewRotation = GetViewMatrix();
         const Matrix34& clientPlayerTM = GetEntity()->GetWorldTM();
         ...
         skipList.clear();
         static ray_hit hit;   
         uint32 flags= rwi_colltype_any(geom_collides)|rwi_force_pierceable_noncoll|rwi_stop_at_pierceable;
		 ...
		 CryLogAlways("testing fuckin");

		if (g_pGameCVars->goc_Crosshair_Mode==1)
		        {		
					
						rAction->getDist(pos, hit.pt);
						...
						if(rAction->ready)
							{
							   ...
							   CMovementRequest moveRequest;
								...
								moveRequest.SetDesiredSpeed(2.0);
								moveRequest.SetMoveTarget(hit.pt);
								 CPlayerMovementController *pPMCp = static_cast<CPlayerMovementController *>(GetMovementController());
								 pPMCp->RequestMovement(moveRequest);
								 rAction->executeGrappleAction();
							}
							*/
						...
						if (pCurrentWeapon && pPlayerWeap->CanFire() && pCurrentWeapon->IsSelected() && !pCurrentWeapon->IsDeselecting())
						{
						Vec3 posi = pCurrentWeapon->GetSlotHelperPos(eIGS_ThirdPerson, "weapon_term", true);
						Matrix33 rotation = pCurrentWeapon->GetSlotHelperRotation(eIGS_ThirdPerson, "weapon_term", true);
						float crossX=fabs(fabs(hit.pt.x)-fabs(posi.x));
						...
						gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(pos,ColorB(255, 128, 128, 255),hit.pt,ColorB(255, 128, 128, 255),6.0f); 
						}
				}
		
         if (g_pGameCVars->cl_tpvMaxWeapDistDebug == 1)
         {
            gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(pos,ColorB(255, 128, 128, 255),hit.pt,ColorB(255, 128, 128, 255),3.0f);
            gEnv->pRenderer->GetIRenderAuxGeom()->DrawSphere(hit.pt, 0.125f, ColorB(255, 128, 128, 255));
         }

         if (hit.dist < 0)
         {
            aimTarget = clientPlayerTM.GetTranslation() + stanceViewOffset + DistanceVec + DistanceLeftRight + DistanceUpDown + curMovementState.aimDirection * WeaponMaxDist;
         }
         else
         {
            aimTarget = hit.pt;
...
void CPlayer::OnGroundEffectAnimEvent(ICharacterInstance* pCharacter, const AnimEventInstance &event)
{
	...
	{
		ISkeletonPose* pSkeletonPose = (pCharacter ? pCharacter->GetISkeletonPose() : 0);
		nJointId = (pSkeletonPose ? pSkeletonPose->GetJointIDByName(event.m_BonePathName) : -1);
	}
