...
/*static */
void	CBodyDamageManager::Warning(const char* szFormat, ...)
{
#ifndef _RELEASE
    ...
    if (IsValidBodyDamageProfileID(profileId) && GetCharacterInfo(characterEntity, characterInfo) && characterInfo.pPhysicalEntity)
        {
            ...
            if (pBodyDamageProfile)
                bResult = pBodyDamageProfile->PhysicalizeEntity(characterInfo.pPhysicalEntity, characterInfo.pSkeletonPose);
        }
