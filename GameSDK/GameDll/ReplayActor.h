...
public:
    ...
    void SetHealthPercentage(int health)
    {
    ...
    int GetReincarnations() const
    {
        return m_reincarnations;
    }
    void SetReincarnations(int reincarnations)
    {
        m_reincarnations = reincarnations;
    }
    ...
    ILINE bool IsFriendly ( ) const
    {
        return m_isFriendly;
    }

    const TPlayerDisplayName& GetName() const
    {
        return m_name;
    }
    void SetName(const TPlayerDisplayName& name)
    {
        m_name = name;
    }
    ...
    bool IsThirdPerson()
    {
        return (m_flags & eRAF_FirstPerson) == 0;
    }

    bool GetIsOnClientsSquad() const
    {
        return ((m_flags&eRAF_IsSquadMember)!=0);
    }
    void SetIsOnClientsSquad(const bool isOnClientsSquad )
    {
        isOnClientsSquad ? m_flags |= eRAF_IsSquadMember : m_flags &= (~eRAF_IsSquadMember);
    }

    bool IsClient() const
    {
        return ((m_flags&eRAF_LocalClient)!=0);
    }
    void SetIsClient(const bool isClient )
    {
        isClient ? m_flags |= eRAF_LocalClient : m_flags &= (~eRAF_LocalClient);
    }

    const QuatT& GetHeadPos() const
    {
        return m_headPos;
    }
    const QuatT& GetCameraTran() const
    {
        return m_cameraPos;
    }
    ...
    {
        return GetEntity()->GetCharacter(ShadowCharacterSlot);
    }

    ILINE SBasicReplayMovementParams& GetBasicMovement()
    {
        return m_basicMovement;
    }
