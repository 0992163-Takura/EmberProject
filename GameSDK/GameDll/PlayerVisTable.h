...
typedef struct SDeferredLinetestReceiver
{
    ...
    ILINE bool IsFree()
    {
        return visBufferIndex == -1;
    }

    void SetInvalid();
    ILINE bool IsValid()
    {
        return visTableIndex != -1;
    }
...
public:
    ...
    inline SVisTableEntry& GetNthVisTableEntry(int32 n)
    {
        return m_visTableEntries[n];
    }
    ...
    void Reset();
    SDeferredLinetestBuffer& GetDeferredLinetestBuffer(int n)
    {
        return m_linetestBuffers[n];
    }

#if ALLOW_VISTABLE_DEBUGGING
    ILINE CPlayerVisTableDebugDraw& GetDebugDraw()
    {
        return m_debugDraw;
    };
#endif
...
private:
    ...
    inline int GetCurrentLinetestBufferTargetIndex()
    {
        return m_currentBufferTarget;
    }
