...
void CTurret::PostInit( IGameObject* pGameObject )
{
	...
	RegisterEvent( ENTITY_EVENT_PREPHYSICSUPDATE, true );
...
int16 CTurret::GetJointIdByName( const char* jointName ) const
{
	...
	const ISkeletonPose* const pSkeletonPose = pCharacterInstance->GetISkeletonPose();
	if ( pSkeletonPose == NULL )
	{
		return -1;
	}

	const int16 jointId = pSkeletonPose->GetJointIDByName( jointName );
	return jointId;
}
...
void CTurret::InitAutoAimParams()
{
	...
	const ISkeletonPose* const pSkeletonPose = pCharacterInstance->GetISkeletonPose();
	IF_UNLIKELY ( pSkeletonPose == NULL )
	{
		return;
	}

	const char* const primaryBoneName = GetProperty< const char* >( pAutoAimParamsTable, "primaryTargetBone", "" );
	m_autoAimParams.primaryBoneId = pSkeletonPose->GetJointIDByName( primaryBoneName );
	...
	m_autoAimParams.secondaryBoneId = pSkeletonPose->GetJointIDByName( secondaryBoneName );
	...
	m_autoAimParams.physicsBoneId = pSkeletonPose->GetJointIDByName( physicsBoneName );
