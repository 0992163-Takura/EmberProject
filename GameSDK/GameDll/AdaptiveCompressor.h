...
public:
    ...
    public:
        ...
        ILINE void WriteBit(int value)
        {
            WriteBit(m_out, m_mask, value);
        }
        ILINE int ReadBit()
        {
            return ReadBit(m_out, m_mask);
        }
        ILINE void WriteVariableLengthValue(int value)
        {
            WriteVariableLengthValue(m_out, m_mask, value);
        }
        ILINE int ReadVariableLengthValue()
        {
            return ReadVariableLengthValue(m_out, m_mask);
        }
        ILINE uint8* GetEnd() const
        {
            return (uint8*)((m_mask==1)?m_out:(m_out+1));
        }
