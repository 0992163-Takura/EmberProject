...
public:
    ...
    int RetrieveHashMethod()
    {
        return m_hashMethod;
    }
    int GetAssetGroupCount()
    {
        return eAG_Num;
    }
    ...
    virtual bool HasDownloadedEssentials(void)
    {
        return true;
    }
    ...
    void SetEnableLogUploads(bool enabled)
    {
        m_enableLogUploads = enabled;
    }
    void Update(float dt);

    bool IsCheatProtectionEnabled(TCheatType cheatType);
    XmlNodeRef GetFileProbeConfiguration()
    {
        return m_fileProtectConfig;
    }

    ILINE float	GetAntiCheatVar	(EAntiCheatVar_Float acvEnum, float fDefault)
    {
        return m_cheatVarsFloat[(int)acvEnum] != FLT_MAX ? m_cheatVarsFloat[(int)acvEnum] : fDefault;
    }
    ILINE int	GetAntiCheatVar	(EAntiCheatVar_Int acvEnum, int iDefault)
    {
        return m_cheatVarsInt[(int)acvEnum] != INT_MAX ? m_cheatVarsInt[(int)acvEnum] : iDefault;
    }
