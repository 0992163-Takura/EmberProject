    ...
    EntityId* GetOtherEntIdPtrForCurMode()
    {
        ...
        switch (mode)
            ...
            case CActor::eASM_Fixed:
                ptr = &dataU.fixed.location;
                break;
            case CActor::eASM_Follow:
                ptr = &dataU.follow.spectatorTarget;
                break;
            case CActor::eASM_Killer:
                ptr = &dataU.killer.spectatorTarget;
                break;
            }
        return ptr;
    }
...
struct SFrameMovementModifiers
{
    ...
    ILINE float GetWeaponSpeedModifier() const
    {
        return weaponSpeedModifier;
    }
    ILINE float GetWeaponRotationModifier() const
    {
        return weaponRotationModifier;
    }
    ILINE float GetTotalSpeedModifier() const
    {
        return totalSpeedModifier;
    }
...
public:
    ...
    ILINE CLocalPlayerComponent* GetLocalPlayerComponent()
    {
        return m_pPlayerTypeComponent;
    }
    ILINE void HoldScreenEffectsUntilNextSpawnRevive()
    {
        m_bDontResetFXUntilNextSpawnRevive = true;
    }
    ...
    ILINE Vec3 GetLastRequestedVelocity() const
    {
    ...
    {
        return (ActorClass)eActorClass_Player;
    }
    ...
    ILINE bool	ShouldPlayIntro() const
    {
        return m_bPlayIntro;
    }
    ...
    ILINE int GetLastInteractionIndex() const
    {
        return static_cast<int>(m_lastCachedInteractionIndex);
    }
    ILINE void SetLastInteractionIndex(int interactionIndex)
    {
        m_lastCachedInteractionIndex = static_cast<int8>(interactionIndex);
    }
    ...
    bool IsStealthKilling() const
    {
        return m_stealthKill.IsBusy();
    }
...
public:
    ...
    virtual EActorSpectatorState GetSpectatorState() const
    {
        return (EActorSpectatorState)m_stats.spectatorInfo.state;
    }
    ...
    {
        return m_stats.spectatorInfo.mode;
    };
    ...
    virtual EntityId GetSpectatorTarget() const
    {
        ...
        return m_stats.spectatorInfo.mode==eASM_Follow?m_stats.spectatorInfo.dataU.follow.spectatorTarget:m_stats.spectatorInfo.dataU.killer.spectatorTarget;
    };
    ...
    virtual EntityId GetSpectatorFixedLocation() const
    {
        assert(m_stats.spectatorInfo.mode==eASM_Fixed);
        return m_stats.spectatorInfo.dataU.fixed.location;
    };

    CPlayerHealthGameEffect& GetPlayerHealthGameEffect()
    {
        return m_playerHealthEffect;
    }
    ...
    ILINE void SetRecentKiller(EntityId killerEid, int damageType)
    {
        m_stats.recentKiller = killerEid;
        m_stats.killedByDamageType = damageType;
    }

    EntityId GetLastAttacker()
    {
        return m_stats.lastAttacker;
    }
    ...
    {
        return m_pPlayerRotation->GetBaseQuat();
    }
    const Quat& GetViewQuat() const
    {
        return m_pPlayerRotation->GetViewQuat();
    }
    ...
    ILINE CPlayerCamera	* GetPlayerCamera() const
    {
    ...
    ILINE SCharacterMoveRequest& GetMoveRequest()
    {
        return m_request;
    }
    ...
    {
        return m_lastTimeDamaged.GetSeconds();
    }
    float GetTimeEnteredLowHealth() const;

    const EntityId GetLastFlashbangShooterId() const
    {
        return m_lastFlashbangShooterId;
    };
    const float GetLastFlashbangTime() const
    {
        return m_lastFlashbangTime;
    }

    const float GetLastZoomedTime() const
    {
        return m_lastZoomedTime;
    }
    ...
    void SetLastReloadTime(float reloadTime)
    {
        m_lastReloadTime = reloadTime;
    };
    ...
    void AddXPBonusMultiplier(int increment)
    {
        m_xpBonusMultiplier += increment;
    }
    int GetXPBonusModifiedXP(int baseXP);

    CPlayerPluginEventDistributor* GetPlayerPluginEventDistributor() const
    {
        return m_pPlayerPluginEventDistributor;
    }

    //Weapon movement modifiers
    ILINE float GetWeaponMovementFactor() const
    {
        return m_frameMovementModifiers.GetWeaponSpeedModifier();
    };
    ILINE float GetWeaponRotationFactor() const
    {
        return m_frameMovementModifiers.GetWeaponRotationModifier();
    };
    ILINE float GetTotalSpeedMultiplier() const
    {
        return m_frameMovementModifiers.GetTotalSpeedModifier();
    };
    ...
    ILINE CPlayerModifiableValues& GetModifiableValues()
    {
        return m_modifiableValues;
    }
    ILINE const CPlayerModifiableValues& GetModifiableValues() const
    {
        return m_modifiableValues;
    }
    ...
    void EnterLargeObjectInteraction(EntityId objectEntityId, const bool bSkipKickAnim = false)
    {
        m_largeObjectInteraction.Enter(objectEntityId, bSkipKickAnim);
    }
    ...
    {
        return m_stats.isInPickAndThrowMode;
    }
    EntityId GetPickAndThrowEntity() const
    {
        return m_stats.pickAndThrowEntity;
    }
    EntityId GetPrevPickAndThrowEntity() const
    {
        return m_stats.prevPickAndThrowEntity;
    }
    ...
    ILINE const CSpectacularKill& GetSpectacularKill() const
    {
        return m_spectacularKill;
    }
    ILINE CSpectacularKill& GetSpectacularKill()
    {
        return m_spectacularKill;
    }
    ILINE const CStealthKill& GetStealthKill() const
    {
        return m_stealthKill;
    }
    ILINE const CLargeObjectInteraction& GetLargeObjectInteraction() const
    {
        return m_largeObjectInteraction;
    }

    ILINE CPickAndThrowProxyConstPtr GetPickAndThrowProxy() const
    {
        return m_pPickAndThrowProxy;
    }
    ILINE CPickAndThrowProxyPtr GetPickAndThrowProxy()
    {
        return m_pPickAndThrowProxy;
    }
    ...
    void SetStealthKilledBy(EntityId shooterId)
    {
        m_stealthKilledById = shooterId;
    }
    EntityId GetStealthKilledBy() const
    {
        return m_stealthKilledById;
    }
    SPlayerRotationParams::EAimType GetCurrentAimType() const;
    const SPlayerRotationParams &GetPlayerRotationParams() const
    {
        return m_playerRotationParams;
    }
    ...
    ILINE void SetLastTimeInLedge( float lastLedgeTime )
    {
        m_lastLedgeTime = lastLedgeTime;
    }
    ...
    {
        return m_pAIAnimationComponent.get();
    }
    ...
    {
        return m_teamWhenKilled;
    }

    ILINE uint8 GetMPModelIndex() const
    {
        return m_mpModelIndex;
    }
    ...
    void RegisterVehicleClient(CVehicleClient* vehicleClient)
    {
        m_pVehicleClient = vehicleClient;
    }
...
private:
    ...
    bool  MountedGunControllerEnabled() const
    {
        return m_mountedGunControllerEnabled;
    }
    void  MountedGunControllerEnabled(bool val)
    {
        m_mountedGunControllerEnabled = val;
    }

    // Support for both AI and players
    void SelectMovementHierarchy();
    ILINE bool IsAIControlled() const
    {
        return m_isAIControlled;
    }; // Note this is NOT necessarily !IsPlayer().
...
protected:
    ...
    bool IsSoundPlaying(EPlayerSounds sound) const
    {
        return m_sounds[sound].audioSignalPlayer.IsPlaying( GetEntity()->GetId() );
    }
...
public:
    ...
    rappleAction * rAction;

#ifdef SERVER_CHECKS
    CShotCounter* GetShotCounter()
    {
        return m_pShotCounter;
    }
#endif // SERVER_CHECKS
