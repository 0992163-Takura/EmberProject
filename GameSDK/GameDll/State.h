...
struct SStateEventData
{
    ...
    int GetInt() const
    {
        assert(m_type==eSEDT_int);
        return m_int;
    }
    float GetFloat() const
    {
        assert(m_type==eSEDT_float);
        return m_float;
    }
    bool GetBool() const
    {
        assert(m_type==eSEDT_bool);
        return m_bool;
    }
    const void* GetPtr() const
    {
        assert( (m_pointer==NULL) || (m_type==eSEDT_voidptr) );
        return m_pointer;
    }
};
...
public:
    ...
    void AddData(const SStateEventData& data)
    {
        m_data.push_back(data);
    }
    const SStateEventData& GetData(uint8 index) const
    {
        return m_data[index];
    }
    ILINE int GetEventId( ) const
    {
        return m_eventType;
    }
    ...
    static SStateEvent CreateStateEvent( int type, const SStateEventData& data )
    {
        SStateEvent event(type);
        event.AddData(data);
        return event;
    }
...
struct SHistory
{
    ...
    ILINE int GetFront() const
    {
        return m_front - 1;
    }
    ILINE int GetBack() const
    {
        return (int)(m_states.size() - 1);
    }
    ILINE const TDebugType& GetStateAt(int index) const
    {
...
struct SStateIndex
{
    ...
    explicit SStateIndex( const char* pName )
        : m_name(HashString(pName)), m_func(), m_parent( NULL ), m_stateID(0), m_hierarchy(0)
    {
        DebugInit( pName );
    }
    SStateIndex( const char* pName, typename CStateProxy<HOST>::StatePtr func, const SStateIndex<HOST>* parent, uint stateID )
        : m_name(HashString(pName)), m_func(func), m_parent(parent), m_stateID((1ULL << static_cast<uint64>( stateID ))), m_hierarchy(0)
    {
        DebugInit( pName );
    ...
    bool operator==( const SStateIndex& rhs ) const
    {
