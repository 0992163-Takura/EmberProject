...
bool CMovementTransitionsController::HandleEvent( const SGameObjectEvent& event )
{
    switch(event.event)
        {
        case eCGE_AllowStartTransitionEnter:
            m_allowedTransitionFlags |= (1<<eTT_Start);
            return true;
        case eCGE_AllowStartTransitionExit:
            m_allowedTransitionFlags &= ~(1<<eTT_Start);
            return true;
        case eCGE_AllowStopTransitionEnter:
            m_allowedTransitionFlags |= (1<<eTT_Stop);
            return true;
        case eCGE_AllowStopTransitionExit:
            m_allowedTransitionFlags &= ~(1<<eTT_Stop);
            return true;
        case eCGE_AllowDirectionChangeTransitionEnter:
            m_allowedTransitionFlags |= (1<<eTT_DirectionChange);
            return true;
        case eCGE_AllowDirectionChangeTransitionExit:
            m_allowedTransitionFlags &= ~(1<<eTT_DirectionChange);
            return true;
        default:
            return false;
