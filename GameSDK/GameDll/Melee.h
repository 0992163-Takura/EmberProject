...
public:
    ...
    {
        return m_attacking || m_netAttacking;
    };

    void NetAttack();
    int GetDamage() const;
    float GetRange()  const;
    ILINE void SetDelayTimer( float timer )
    {
        m_delayTimer = timer;
    }
    ...
    ILINE static const char* GetWeaponComponentType()
    {
        return "Melee";
    }
    void OnMeleeHitAnimationEvent();

    ILINE const SMeleeParams& GetMeleeParams() const
    {
        return m_pMeleeParams->meleeparams;
    }
    ILINE const SMeleeModeParams* GetMeleeModeParams() const
    {
        return m_pMeleeParams;
    }

    static void PlayHitMaterialEffect(const Vec3 &position, const Vec3 &normal, bool bBoostedMelee, int surfaceIdx);
    ILINE static void SetMeleeDelay(float fDelay)
    {
        s_fNextAttack = fDelay;
    }

    float GetDuration() const;
    float GetDelay() const;

    ILINE bool IsMeleeWeapon() const
    {
        return m_pMeleeParams->meleeparams.is_melee_weapon;
    }
