...
struct SKillCamStreamData
{
    ...
    bool IsUnused() const
    {
        static SKillCamExpectedData blank;
        return m_expect==blank;
    }
    ILINE void Validate()
    {
        m_validExpectationsTimer = 0.f;
    }
    ILINE void SetValidationTimer( const float time )
    {
        m_validExpectationsTimer = time;
    }
    ILINE bool IsValidationTimerSet() const
    {
        return m_validExpectationsTimer>0.f;
    }
    ILINE bool UpdateValidationTimer( const float dt )
    {
        if( m_validExpectationsTimer>0.f )
            {
                m_validExpectationsTimer=(float)__fsel(m_validExpectationsTimer-dt,m_validExpectationsTimer-dt,0.f);
                return m_validExpectationsTimer>0.f;
            }
        return true;
    }
    ILINE size_t GetFinalSize() const
    {
        return m_finalSize;
    }
...
struct SHighlightRules
{
    struct SFactor
    {
        ...
        ILINE void Apply( float& in ) const
        {
            in = (in*mul)+add;
        }
        float add;
    ...
    struct SFactorMap
    {
        ...
        void SetDefault( const float _add, const float _mul )
        {
            m_default.add=_add;
            m_default.mul=_mul;
        }
        void Add( const T& value, const float add, const float mul )
        {
            m_map[value] = SFactor(add, mul);
        }
...
public:
    ...
    struct Timings
    {
        ...
        ILINE void Reset()
        {
            recStart=recEnd=relDeathTime=0.f;
        }
        ILINE void ApplyOffset( const float offset )
        {
            recStart+=offset;
            recEnd+=offset;
        }
