...
public:
    ...
    ILINE EntityId  GetOwner() const
    {
        return m_OwnerId;
    }
    ILINE bool ShouldShowShieldedHitIndicator() const
    {
        return m_bShowShieldedHitIndicator;
    }
    ...
    const Vec3& GetInitialThrowPos() const
    {
        return m_initialThrowPos;
    }
...
#ifndef _RELEASE
    void DebugSetThrowPath(const Vec3& start, const Vec3& end )
    {
        m_throwOrigin = start;
        m_throwTarget = end;
    }
    ...
    ILINE void OnCollision(const EventPhysCollision& collision)
    {
        m_intersectionAssist.OnCollision(collision);
    }

    // Intersection Assist
    CIntersectionAssistanceUnit& GetIntersectionAssistUnit()
    {
        return m_intersectionAssist;
    }
