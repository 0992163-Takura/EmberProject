...
public:
    ...
    static ILINE bool KillCamEnabled()
    {
        return g_pGameCVars->kc_enable && !g_pGameCVars->g_modevarivar_disableKillCam;
    }
    ...
    ILINE bool IsPlaybackQueued() const
    {
        return m_playbackState == ePS_Queued;
    }
    ILINE bool IsPlayingBack() const
    {
        return m_playbackState == ePS_Playing;
    }
    ILINE bool IsPlayingHighlightsReel() const
    {
        return (m_bShowAllHighlights || m_highlightsReel);
    }
    ILINE bool IsPlayingOrQueued() const
    {
        return m_playbackState == ePS_Playing || m_playbackState == ePS_Queued;
    }
    ILINE bool IsRecording() const
    {
        return m_recordingState == eRS_Recording;
    }
    ILINE bool IsRecordingAndNotPlaying() const
    {
        return m_recordingState == eRS_Recording && m_playbackState != ePS_Playing;
    }

    bool PlayWinningKillcam()
    {
        m_bCanPlayWinningKill = m_hasWinningKillcam?true:m_bCanPlayWinningKill;
        return m_hasWinningKillcam;
    }
    bool HasWinningKillcam() const
    {
        return m_hasWinningKillcam;
    }
    ...
    void SetPlayerVelocity(EntityId eid, Vec3 vel)
    {
        m_chrvelmap[eid] = vel;
    }
    void SetPlayerAimIK(EntityId eid, AimIKInfo aikinf)
    {
        m_eaikm[eid] = aikinf;
    }
    ...
    ILINE CKillCamGameEffect & GetKillCamGameEffect ( )
    {
        return m_killCamGameEffect;
    }
    ...
    ILINE bool IsInBulletTime() const
    {
        return m_playInfo.m_view.currView==SPlaybackInfo::eVM_BulletTime;
    }
    ...
    ILINE const SPlaybackInfo& GetPlaybackInfo() const
    {
        return m_playInfo;
    }
...
private:
    ...
    struct SStackGuard
    {
        SStackGuard(int& counter) : m_counter(counter)
        {
            counter++;
        }
        ~SStackGuard()
...
#define REPLAY_EVENT_GUARD CRecordingSystem::SStackGuard __ReplayEventGuard__(CRecordingSystem::m_replayEventGuard);
    ...
    ILINE bool FilterItemSuffix(const char* suffix)
    {
        return strcmp(suffix, "flag") == 0 || strcmp(suffix, "tick") == 0;
    }
    ...
    class CRecordingSystemDebug* GetDebug() const
    {
        return m_pDebug;
    }
