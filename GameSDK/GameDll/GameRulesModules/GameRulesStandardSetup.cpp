...
void CGameRulesStandardSetup::OnPlayerRevived(EntityId playerId)
{
	...
	if (pActor)
	{
		...
		static IEntityClass *pBinoClass = gEnv->pEntitySystem->GetClassRegistry()->FindClass("Binoculars");
		...
		if(pInventory)
		{
			...
			BinoId = pInventory->GetItemByClass(pBinoClass);
		...
		if(!BinoId)
			BinoId = pItemSystem->GiveItem(pActor, "Binoculars", false, false, false);
		...
		CRY_ASSERT_MESSAGE(pItemSystem->GetItem(BinoId), "Failed to find CItem for Binoculars");
