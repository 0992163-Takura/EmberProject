...
public:
    ...
    virtual CryCriticalSection* GetNetworkLock()
    {
        return &m_networkLock;
    }
    ...
    const SVehiclePhysicsStatus& GetPhysicsStatus(unsigned int thread)
    {
        CRY_ASSERT(thread<k_numThreads);
        return m_physStatus[thread];
    }
