    ...
    if (pCharacterInstance)
        {
            ...
            else if ((partId >= 0) && (partId < (int)pSkeletonPose->GetJointCount()))
                {
                    const char* boneName = pSkeletonPose->GetJointNameByID(pSkeletonPose->getBonePhysParentOrSelfIndex(partId));
                    if ((boneName != NULL))
                        {
...
CBodyDamageProfile::JointId CBodyDamageProfile::JointId::GetJointIdFromPartId( ISkeletonPose& skeletonPose, const int partId )
{
    if ((partId >= 0) && (partId < CBodyDamageProfile::ATTACHMENT_BASE_ID))
        {
            const char* boneName = skeletonPose.GetJointNameByID(skeletonPose.getBonePhysParentOrSelfIndex(partId));
            if ((boneName != NULL))
                {
                    return JointId(boneName);
                }
...
CBodyDamageProfile::MatMappingId CBodyDamageProfile::MatMappingId::GetMatMappingIdFromPartId( ISkeletonPose& skeletonPose, const int partId )
{
    if ((partId >= 0) && (partId < CBodyDamageProfile::ATTACHMENT_BASE_ID))
        {
            const char* boneName = skeletonPose.GetJointNameByID(skeletonPose.getBonePhysParentOrSelfIndex(partId));
...
CBodyDamageProfile::CEffectiveMaterials::CEffectiveMaterials(CBodyDamageProfile& bodyDamageProfile, ISkeletonPose& skeletonPose, IPhysicalEntity& physicalEntity)
            ...
            if (0 == strcmp(node->getTag(), "EffectiveMaterial"))
            ...
            else if (0 == strcmp(node->getTag(), "Bone"))
                {
                    ...
                    if (node->getAttr("name", &boneName1))
                        {
                            int namedBoneId = m_skeletonPose.GetJointIDByName(boneName1);
    ...
    for (int partIndex = 0; partIndex < nParts; partIndex++)
        {
            ...
            if (m_physicalEntity.GetParams(&part) && part.partid < CBodyDamageProfile::ATTACHMENT_BASE_ID)
                {
                    const JointId jointId = JointId::GetJointIdFromPartId( m_skeletonPose, part.partid );
                    const MatMappingId matMappingId = MatMappingId::GetMatMappingIdFromPartId( m_skeletonPose, part.partid );
...
bool CBodyDamageProfile::PhysicalizeEntity(IPhysicalEntity* pPhysicalEntity, ISkeletonPose* pSkeletonPose) const
{
    assert(pPhysicalEntity);
    assert(pSkeletonPose);
    bool bResult = (pPhysicalEntity != NULL) && (pSkeletonPose != NULL);
            ...
            for (TMaterialMappingEntries::const_iterator it = m_effectiveMaterialsMapping.begin(); it != itEnd; ++it)
                {
                    ...
                    uint16 boneId = pSkeletonPose->GetJointIDByName(it->first.GetName());
...
void CBodyDamageProfile::CPart::LoadElements(
    const XmlNodeRef& partNode, ISkeletonPose& skeletonPose,
...
void CBodyDamageProfile::CPart::AddBone(const XmlNodeRef& boneNode, const char* boneName, ISkeletonPose& skeletonPose, CEffectiveMaterials& effectiveMaterials)
{
    const int16 boneId = skeletonPose.GetJointIDByName(boneName);
    ...
    if (!m_bInitialized || bReload)
        {
            if (m_partsRootNode && m_damageRootNode)
                {
                    CEffectiveMaterials effectiveMaterials(*this, *characterInfo.pSkeletonPose, *characterInfo.pPhysicalEntity);

                    if (loadEffectiveMaterials)
                        effectiveMaterials.Load(m_damageRootNode);

                    LoadParts(m_partsRootNode, *characterInfo.pSkeletonPose, *characterInfo.pAttachmentManager, effectiveMaterials);
                    LoadMultipliers(m_damageRootNode);
                    LoadImpulseFilters(m_damageRootNode, *characterInfo.pSkeletonPose);
...
void CBodyDamageProfile::LoadImpulseFilters(const XmlNodeRef& rootNode, ISkeletonPose& skeletonPose)
{
...
void CBodyDamageProfile::LoadParts(const XmlNodeRef& rootNode, ISkeletonPose& skeletonPose, IAttachmentManager& attachmentManager, CEffectiveMaterials& effectiveMaterials)
{
...
void CBodyDamageProfile::LoadImpulse( const XmlNodeRef& filterNode, ISkeletonPose& skeletonPose, const PartId partID )
{
...
void CBodyDamageProfile::LoadImpulseFilter(const XmlNodeRef& filterNodeRoot, ISkeletonPose& skeletonPose)
{
    const char* partName = NULL;
                    ...
                    for( ; (iFilter!=iEnd) && (iFilter->first == partID); ++iFilter )
                        {
                            ...
                            else if( iFilter->second.projectileClassID == uint16(~0) )
                                {
                                    impulseFilter = iFilter->second;
                                    bFound = true;
                                }
