...
public:
    ...
    ILINE void BindToTracer(int8 tracerIdx)
    {
        m_boundToTracerIdx = tracerIdx;
    }
    ILINE int  GetTracerIdx() const
    {
        return m_boundToTracerIdx;
    }
    ILINE void BindToThreatTrailTracer(int8 tracerIdx)
    {
        m_threatTrailTracerIdx = tracerIdx;
    }
    ILINE int  GetThreatTrailTracerIdx() const
    {
        return m_threatTrailTracerIdx;
    }

    CWeapon *GetWeapon() const;
    ILINE EntityId	GetWeaponId() const
    {
        return m_weaponId;
    }
    ILINE EntityId	GetOwnerId()const
    {
        return m_ownerId;
    }
    ILINE float			GetSpeed() const
    {
        return m_pAmmoParams->speed;
    }
    ILINE bool			IsLinked() const
    {
        return CheckAllProjectileFlags(ePFlag_linked);
    }
    ILINE void			SetHitReported()
    {
        SetProjectileFlags(ePFlag_hitRecorded);
    }
    ...
    {
        return *m_pAmmoParams;
    }

    inline void SetProjectileFlags(uint32 flags, bool on)
    {
        m_projectileFlags = on ? (m_projectileFlags | flags) : (m_projectileFlags & ~flags);
    }
    inline void SetProjectileFlags(uint32 flags)
    {
        m_projectileFlags |= flags;
    }
    inline void ClearProjectileFlags(uint32 flags)
    {
        m_projectileFlags &= ~flags;
    }
    inline bool CheckAnyProjectileFlags(uint32 flags) const
    {
        return (m_projectileFlags & flags) ? true : false;
    }
    inline bool CheckAllProjectileFlags(uint32 flags) const
    {
        return (m_projectileFlags & flags) == flags;
    }
    ...
    const Vec3& GetInitialPos() const
    {
    ...
    const CTimeValue& GetSpawnTime() const
    {
        return m_spawnTime;
    }
    ...
    void MultiplyDamage(float multipier)
    {
        m_damage = (int)((float)m_damage * multipier);
    }
    ...
    ILINE void	SetShotId(uint32 sid)
    {
        m_shotId = sid;
    }
    ILINE uint32	GetShotId()
    {
        return m_shotId;
    }
#endif

    static SMaterialLookUp&  GetMaterialLookUp()
    {
        return s_materialLookup;
    }
...
protected:
    ...
    ILINE bool RequiresDelayedDestruct() const
    {
        return (m_pAmmoParams && (m_pAmmoParams->mpProjectileDestructDelay > 0.f));
    }
