            ...
            if( ISkeletonPose* pSkelPose = pCharacter->GetISkeletonPose() )
                {
                    ...
                    m_autoAimParams.primaryBoneId = pSkelPose->GetJointIDByName(primaryBoneName);

                    const char* secondaryBoneName;
                    table.getAttr("SecondaryBoneName", &secondaryBoneName);
                    m_autoAimParams.secondaryBoneId = pSkelPose->GetJointIDByName(secondaryBoneName);

                    const char* physicsBoneName;
                    table.getAttr("PhysicsBoneName", &physicsBoneName);
                    m_autoAimParams.physicsBoneId = pSkelPose->GetJointIDByName(physicsBoneName);
