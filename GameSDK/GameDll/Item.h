...
public:
    ...
    void UnlowerItem();
    ILINE bool IsLowered() const
    {
        return m_itemLowerMode != eILM_Raised;
    };
    eItemLowerMode GetLowerMode() const
    {
        return m_itemLowerMode;
    }
    ...
    {
        return IsRippedOff() ? !IsUsed() : !IsMounted();
    }
    ...
    {
        return AreAnyItemFlagsSet(eIF_Modifying | eIF_Transitioning);
    }
    ...
    {
        CRY_ASSERT_MESSAGE(0, "DEPRECATED: Use RemoveAccessory(const IEntityClass* pClass)");
    };
    ...
    const TAccessoryArray& GetAccessories() const
    {
    ...
    {
        return AreAnyItemFlagsSet(eIF_NoDrop);
    }
    ...
    ILINE void SetItemFlags(TItemFlags flags)
    {
        m_itemFlags |= flags;
    }
    ILINE void ClearItemFlags(TItemFlags flags)
    {
        m_itemFlags &= ~flags;
    }
    ILINE void SetItemFlag(TItemFlags flags, bool bSet)
    {
        if(bSet) SetItemFlags(flags);
        else ClearItemFlags(flags);
    }
    ILINE bool AreAnyItemFlagsSet(TItemFlags flag)	const
    {
        return (m_itemFlags & flag) != 0;
    }
    ...
    ILINE bool IsSelectGrabbingWeapon() const
    {
        return AreAnyItemFlagsSet(eIF_SelectGrabbingWeapon);
    }
    ...
    ILINE IActionController* GetActionController() const
    {
        return m_pCurrentActionController;
    }
    ...
    {
        return *m_pCurrentManItemParams;
    }
    const SMannequinItemParams::FragmentIDs& GetFragmentIds() const
    {
        return GetMannequinItemParams().fragmentIDs;
    }
    ...
    ILINE void OnItemSelectActionComplete()
    {
        m_pSelectAction.reset();
    }
    ILINE void ForceSelectActionComplete ()
    {
        if(m_pSelectAction)
            {
                m_pSelectAction->ItemSelectCancelled();
            }
        OnItemSelectActionComplete();
    }
...
protected:
    ...
    public:
        ...
        ILINE EntityId GetId() const
        {
            return m_id;
        }
        ILINE const CActorWeakPtr& GetActorWeakPtr() const
        {
            return m_pActor;
        }
