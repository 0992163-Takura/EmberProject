...
void CGameRules::CalculateExplosionAffectedEntities(const ExplosionInfo &explosionInfo, TExplosionAffectedEntities& affectedEntities )
{
    ...
    const float kMaxAllowedDamageRadius = 12.0f;
    const float maxDamageRadius = min(explosionInfo.radius, kMaxAllowedDamageRadius);
    const float minDamageRadius = min(explosionInfo.minRadius, maxDamageRadius);

#if !defined(_RELEASE)
    const bool damageRadiusTooBig = (explosionInfo.radius > (kMaxAllowedDamageRadius + 0.01f));
    CRY_ASSERT_MESSAGE(!damageRadiusTooBig, "Explosion Damage radius too big, clamping to max value allowed");
    if (damageRadiusTooBig)
        {
            GameWarning("Explosion Damage radius requested ('%.2f'meters) too big, clamping to default max value ('%.2f'meters)", explosionInfo.radius, kMaxAllowedDamageRadius);
        }
#endif

    pe_explosion explosion;
    explosion.epicenter = explosionInfo.pos;
    explosion.rmin = minDamageRadius;
    explosion.rmax = max(maxDamageRadius, 0.0001f);
    explosion.r = minDamageRadius;
                            ...
                            if(IEntity *pEntity = pVehicle->GetEntity())
                                {
                                    ...
                                    if (pEnt && aabb.GetDistanceSqr(explosionInfo.pos) <= maxDamageRadius*maxDamageRadius)
                                        {
    ...
    const float kMaxAllowedPhysicsRadius = 5.0f;
    const float maxPhysicsRadius = min(explosionInfo.physRadius, kMaxAllowedPhysicsRadius);
    const float minPhysicsRadius = min(explosionInfo.minPhysRadius, maxPhysicsRadius);

#if !defined(_RELEASE)
    const bool physicsRadiusTooBig = (explosionInfo.physRadius > (kMaxAllowedPhysicsRadius + 0.01f));
    CRY_ASSERT_MESSAGE(!physicsRadiusTooBig, "Explosion Physics radius too big, clamping to max value allowed");
    if (physicsRadiusTooBig)
        {
            GameWarning("Explosion Physics radius requested ('%.2f'meters) too big, clamping to default max value ('%.2f'meters)", explosionInfo.physRadius, kMaxAllowedPhysicsRadius);
        }
#endif

    explosion.rmin = minPhysicsRadius;
    explosion.rmax = max(maxPhysicsRadius, 0.0001f);
    explosion.r = minPhysicsRadius;
