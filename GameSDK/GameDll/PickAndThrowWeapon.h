...
class CPickAndThrowWeapon :	public CWeapon, public IMeleeCollisionHelperListener, public IEntityPoolListener
{
    ...
    struct HelperInfo
    {
        ...
        ILINE bool Valid() const
        {
            return (m_pSubObject != NULL);
        }
...
public:
    ...
    bool ShieldsPlayer()
    {
        return m_objectId!=0 && GetGrabTypeParams().shieldsPlayer;
    }
    ...
    ILINE bool IsIdling() const
    {
        return (m_state == eST_IDLE) && !IsNPC();
    }
...
private:
    ...
    CActor* GetNPCGrabbedActor()
    {
        return const_cast<CActor*>(static_cast<const CPickAndThrowWeapon*>(this)->GetNPCGrabbedActor());
    }
    ...
    bool IsBusy() const
    {
        return IsNPC() || CWeapon::IsBusy();
    }
    bool CanSprint() const
    {
        return gEnv->bMultiplayer ? !IsBusy() : true;
    }
