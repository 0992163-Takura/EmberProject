...
namespace BucketAllocatorDetail
{
	...
	struct SyncPolicyLocked
	{
		...
		typedef CryCriticalSectionNonRecursive OSLock;

		UserLock m_cleanupLock;
		// On PS3 use a proper critical section for the refill to avoid deadlocks due to threads being preempted whilst the lock is held
		OSLock m_refillLock;
		...
		struct CleanupReadLock
		{
			ILINE CleanupReadLock(SyncPolicyLocked& policy)
				: m_policy(policy)
				, m_ownsRefillLock(false)
			{
				while (policy.m_cleanupLock.lock)
				{
					if (!m_ownsRefillLock)
					{
						m_policy.m_refillLock.Lock();
						m_ownsRefillLock = true;
					}
				}
			}
			~CleanupReadLock()
			{
				IF_UNLIKELY (m_ownsRefillLock)
				{
					m_policy.m_refillLock.Unlock();
				}
			}

		private:
			SyncPolicyLocked& m_policy;
			bool m_ownsRefillLock;
		};
		...
		struct RefillLock
		{
			RefillLock(SyncPolicyLocked& policy) : m_policy(policy) { m_policy.m_refillLock.Lock(); }
			~RefillLock() { m_policy.m_refillLock.Unlock(); }
