...
{	
	...
	// Summary:
	// Ensure that the ComputeVertices Jobs have finished for the provided thread id
	virtual void SyncComputeVerticesJobQueues( uint32 nThreadId ) = 0;

	// Summary:
	// Set the number of ComputeVerticesJobs we are going to start
	virtual void SetNumComputeVerticesJobs( int nNumComputeVerticesJobs, int nThreadId) = 0;

	// Summary:
	// Get the SyncObject for the ComputeVertices Jobs
	virtual JobManager::SJobState* GetComputeVerticesJobState( uint32 nThreadId ) = 0;

	// Summary:
	// Set the number of ParticleContainer which are to be rendered int he general pass
	virtual void SetNumContainerToRender( int nNumContainer ) = 0;

	virtual void SetNumParticlesCulled( int nNum ) = 0;
	// </interfuscator:shuffle>
};
