...
struct SRenderingPassInfo
{
	...
	SRenderingPassInfo( const SRenderingPassInfo &rOther );
	...
	static SRenderingPassInfo CreateGeneralPassRenderingInfo( const CCamera &rCamera, uint32 nRenderingFlags = DEFAULT_FLAGS );
...
{ 
	return m_nRenderStackLevel == 0 && static_cast<EShadowMapType>(m_eShadowMapRendering) == SHADOW_MAP_NONE; 
}
...
////////////////////////////////////////////////////////////////////////////////
inline SRenderingPassInfo SRenderingPassInfo::CreateGeneralPassRenderingInfo( const CCamera &rCamera, uint32 nRenderingFlags)
{
	...
	passInfo.m_nRenderStackLevel = 0;
	...
	passInfo.m_nThreadID =  static_cast<uint8>(nThreadID);
	passInfo.m_nRenderFrameID = gEnv->pRenderer->GetFrameID();
	passInfo.m_nRenderMainFrameID = gEnv->pRenderer->GetFrameID(false);
	passInfo.m_pCamera = gEnv->p3DEngine->GetRenderingPassCamera(rCameraToSet);
	passInfo.m_bCameraUnderWater = gEnv->p3DEngine->IsUnderWater(rCameraToSet.GetPosition());
	passInfo.InitRenderingFlags(nRenderingFlags);

	// update general pass zoom factor
	int nZoomMode = gEnv->p3DEngine->GetZoomMode();	
	float fPrevZoomFactor = gEnv->p3DEngine->GetPrevZoomFactor();
	
	float fZoomFactor = 0.2f + 0.8f*(RAD2DEG(rCameraToSet.GetFov())/60.f);		
	int bZoomInProgress = nZoomMode && fabs(fPrevZoomFactor - fZoomFactor) > 0.02f;

	const float fZoomThreshold = 0.7f;
	if(fZoomFactor < fZoomThreshold)  
      nZoomMode++;  
  else  
      nZoomMode--;
  
	// clamp zoom mode into valid range
	nZoomMode = clamp_tpl<int>(nZoomMode,0,4);

	// store information for next frame in 3DEngine
	gEnv->p3DEngine->SetPrevZoomFactor(fZoomFactor);
	gEnv->p3DEngine->SetZoomMode(nZoomMode);

	passInfo.m_fZoomFactor = fZoomFactor;
	passInfo.m_nZoomInProgress = bZoomInProgress;
	passInfo.m_nZoomMode = nZoomMode;
...
inline SRenderingPassInfo SRenderingPassInfo::CreateRecursivePassRenderingInfo( const CCamera &rCamera, uint32 nRenderingFlags )
{
	...
	passInfo.m_eShadowMapRendering = static_cast<uint8>(SRenderingPassInfo::SHADOW_MAP_NONE);
	threadID nThreadID = 0;
	gEnv->pRenderer->EF_Query(EFQ_MainThreadList, nThreadID);
	passInfo.m_nThreadID = static_cast<uint8>(nThreadID);
	passInfo.m_nRenderFrameID = gEnv->pRenderer->GetFrameID();
	passInfo.m_nRenderMainFrameID = gEnv->pRenderer->GetFrameID(false);
	passInfo.m_pCamera = gEnv->p3DEngine->GetRenderingPassCamera(rCamera);
	passInfo.m_bCameraUnderWater = gEnv->p3DEngine->IsUnderWater(rCamera.GetPosition());

	// adjust view distance in recursive mode by adjusting the ZoomFactor
	float fZoomFactor = 0.2f + 0.8f*(RAD2DEG(rCamera.GetFov())/60.f);
	passInfo.m_fZoomFactor = fZoomFactor / pRecursionViewDistRatio->GetFVal();
	passInfo.m_nZoomInProgress = 0;
	passInfo.m_nZoomMode = 0;
...
inline SRenderingPassInfo SRenderingPassInfo::CreateShadowPassRenderingInfo( const CCamera &rCamera, SRenderingPassInfo::EShadowMapType eShadowMapType, uint32 nRenderingFlags )
{
	...
	passInfo.m_pCamera = gEnv->p3DEngine->GetRenderingPassCamera(rCamera);
	passInfo.m_bCameraUnderWater = gEnv->p3DEngine->IsUnderWater(rCamera.GetPosition());
	passInfo.m_fZoomFactor = 0.2f + 0.8f*(RAD2DEG(rCamera.GetFov())/60.f);
	passInfo.m_nZoomInProgress = 0;
	passInfo.m_nZoomMode = 0;
	passInfo.InitRenderingFlags(nRenderingFlags);
...
{
	SRenderingPassInfo passInfo;
	passInfo.m_nRenderStackLevel = rPassInfo.m_nRenderStackLevel;
	passInfo.m_eShadowMapRendering = rPassInfo.m_eShadowMapRendering;
	passInfo.m_nThreadID = rPassInfo.m_nThreadID;
	passInfo.m_nRenderFrameID = rPassInfo.m_nRenderFrameID;
	passInfo.m_nRenderMainFrameID = rPassInfo.m_nRenderMainFrameID;
	passInfo.m_nRenderingFlags = rPassInfo.m_nRenderingFlags;
...
inline SRenderingPassInfo SRenderingPassInfo::CreateTempRenderingInfo( uint32 nRenderingFlags, const SRenderingPassInfo &rPassInfo )
{
	...
	passInfo.m_pCamera = rPassInfo.m_pCamera;
	passInfo.m_bCameraUnderWater = rPassInfo.m_bCameraUnderWater;
	passInfo.m_fZoomFactor = rPassInfo.m_fZoomFactor;
	passInfo.m_nZoomInProgress = rPassInfo.m_nZoomInProgress;
	passInfo.m_nZoomMode = rPassInfo.m_nZoomMode;

	passInfo.m_nRenderingFlags = nRenderingFlags;

	passInfo.pShadowGenMask = rPassInfo.pShadowGenMask;
	passInfo.nShadowFrustumId = rPassInfo.nShadowFrustumId;
	passInfo.nSide = rPassInfo.nSide;

	return passInfo;
}

///////////////////////////////////////////////////////////////////////////////
inline SRenderingPassInfo::SRenderingPassInfo( const SRenderingPassInfo &rOther )
{
	m_nRenderStackLevel = rOther.m_nRenderStackLevel;
	m_eShadowMapRendering = rOther.m_eShadowMapRendering;
	m_nThreadID = rOther.m_nThreadID;	
	m_nRenderFrameID = rOther.m_nRenderFrameID;
	m_nRenderMainFrameID = rOther.m_nRenderMainFrameID;
	m_pCamera = rOther.m_pCamera;
	m_bCameraUnderWater = rOther.m_bCameraUnderWater;
	m_fZoomFactor = rOther.m_fZoomFactor;	
	m_nZoomInProgress = rOther.m_nZoomInProgress;
	m_nZoomMode = rOther.m_nZoomMode;
	m_nRenderingFlags = rOther.m_nRenderingFlags;

	pShadowGenMask = rOther.pShadowGenMask;
	nShadowFrustumId = rOther.nShadowFrustumId;
	nSide = rOther.nSide;
}
