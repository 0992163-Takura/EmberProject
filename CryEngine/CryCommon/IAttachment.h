...
struct IAttachment
{
	...
	virtual void UpdatePendulumAndJoint() =0;
	virtual void DrawAttachment(SRendParams& rParams, const SRenderingPassInfo &passInfo,const Matrix34& WorldMat34, f32 fZoomFactor=1)=0;
	virtual ~IAttachment(){}
	// </interfuscator:shuffle>
};
...
struct CSKINAttachment : public IAttachmentObject
{
  ...
  void Release()  {  delete this; }
	void ProcessAttachment( IAttachment *pIAttachment )	
	{	
		if (m_pIAttachmentSkin)	
			m_pIAttachmentSkin->ProcessAttachment( pIAttachment ); 
	};
...
struct ICharacterPartAttachment : public CSKELAttachment
{
  // Visibility mask
  virtual uint32 GetVisibilityMask() const = 0;
  virtual void SetVisibilityMask(uint32 newVisMask) = 0;
  virtual void AddFaceAttachment(const char* partName,const char* name, QuatT attRelativeDefault, QuatT attAbsoluteDefault, ClosestTri face) = 0;
};
