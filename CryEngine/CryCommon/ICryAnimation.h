...
//Description
//
// Placeholder representation of a character which can be used for pre-fetching model (skins and skeletons) purposes at loading time
UNIQUE_IFACE struct IPlaceholderCharacter
{
	// <interfuscator:shuffle>
	virtual ~IPlaceholderCharacter(){}

	virtual void AddRef() = 0;
	virtual void Release() = 0;

	virtual void RequestHighPriorityMaterialsLoading() = 0;
	// </interfuscator:shuffle>
};
...
UNIQUE_IFACE struct ICharacterModel
{
	...
	virtual uint32 GetNumInstances() const = 0; // Return number of instances that reference this model.
	...
	virtual const char* GetNamePART() const = 0;			// Returns type name given to the model on creation time.
	
	virtual phys_geometry* GetJointPhysGeom( uint32 jointIndex, int nLod ) = 0;
	virtual const phys_geometry* GetJointPhysGeom( uint32 jointIndex, int nLod ) const = 0;

	virtual ICharacterInstance* GetICharInstanceFromModel( uint32 num ) const = 0;
	virtual ICharacterModelSkeleton* GetICharacterModelSkeleton() const = 0;
	...
	virtual uint32 GetNumJoints() const = 0;
